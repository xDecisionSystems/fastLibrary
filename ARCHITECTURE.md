...
| `venue_long`       | string          | `$set`              | Full venue/proceedings title (used for BibTeX `booktitle`) |
...
| `is_best_paper`    | bool            | `$set`              | Strategy/searcher metadata flag     |
| `presentation_url` | string          | `$set`              | Optional presentation link          |
| `tags`             | array[string]   | `$set`              | User-assigned tags                 |
| `ingested`         | bool            | `$set`              | Whether RAG has ingested this paper|
| `doi_synthetic`    | bool            | `$set`              | True when DOI is generated by strategy |
| `bibtex`           | string          | `$set`              | Generated BibTeX entry when strategy includes `build_bibtex` |
...
| `SEARCHER_API_BASE_URL` | No | `https://searcher.xds-lab.com/aev/search` | External searcher API base URL used by conference paper search/download actions |
